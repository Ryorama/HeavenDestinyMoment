...
import com.xiaohunao.heaven_destiny_moment.common.moment.moment.instance.SimpleKillEntityInstance;
...
public class SimpleKillEntityMoment extends Moment{
    public static final MapCodec<SimpleKillEntityMoment> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(
    ...
    ).apply(instance, SimpleKillEntityMoment::new));


    public SimpleKillEntityMoment() {
        super();
    }

    public SimpleKillEntityMoment(Optional<IBarRenderType> renderType, Optional<Area> area, Optional<MomentData> momentDataContext,
    ...
    public MomentInstance newMomentInstance(Level level, Moment momentResourceKey) {
        return new SimpleKillEntityInstance(level,momentResourceKey);
    }


    @Override
    public MapCodec<? extends Moment> codec() {
        return HDMMomentRegister.SIMPLE_KILL_ENTITY_MOMENT.get();
    }
}
