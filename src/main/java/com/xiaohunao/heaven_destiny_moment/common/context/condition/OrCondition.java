...
public record OrCondition(ICondition or, List<ICondition> trueCondition, List<ICondition> falseCondition) implements ICondition {
    ...
    public boolean matches(MomentInstance instance, @Nullable MomentState tryModifyState, @Nullable BlockPos pos, @Nullable ServerPlayer serverPlayer) {
        if (or.matches(instance, tryModifyState, pos, serverPlayer)) {
            return trueCondition.stream().allMatch(condition -> condition.matches(instance, tryModifyState, pos, serverPlayer));
        } else {
            return falseCondition.stream().allMatch(condition -> condition.matches(instance, tryModifyState, pos, serverPlayer));
        }
    }
